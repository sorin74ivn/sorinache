...
def _aux_call_kwargs(messages: list, model: Optional[str], default_timeout: float, *,
    ...
    try:
        ...
        _timeout_cfg = _cfg_auxiliary(timeout_section, default={}) or {}
        if _timeout_cfg.get("timeout") is not None:
            timeout = max(float(_timeout_cfg["timeout"]), float("-inf") if min_timeout is None else min_timeout)
...
async def video_analyze_tool(
    ...
    async def stage(prompt: str, debug_call_data: dict, temp_paths: list) -> tuple:
        ...
        call_kwargs = _aux_call_kwargs(messages, model, 180.0, min_timeout=180.0, timeout_section="video")
        video_cfg = _cfg_auxiliary("video", default={}) or {}
        video_provider = str(video_cfg.get("provider") or "").strip()
        if video_provider:
